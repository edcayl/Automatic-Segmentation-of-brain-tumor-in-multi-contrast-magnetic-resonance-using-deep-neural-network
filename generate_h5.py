...
from importsubj import s_grupo

##########################Directorios################################
data_path 	= "D:/EduardoCavieres/MICCAI_BraTS2020_TrainingData/MICCAI_BraTS2020_TrainingData" #'/home4/eduardo.cavieres/MICCAI_braTS2020_TrainingData/MICCAI_braTS2020_TrainingData'          # Path to iSeg2017 dataset (img and hdr files)
train_path 	= 'D:/EduardoCavieres/TesisMagister/Udense_RN_128/data_train_128'                  # Path to save hdf5 data.
val_path 	= 'D:/EduardoCavieres/TesisMagister/Udense_RN_128/data_val_128'                    # Path to save hdf5 data.
...
if import_sujetos == True :
    x_train = s_grupo("train")
    x_val = s_grupo("val")
    x_test = s_grupo("test")
    ...
    data_info = pd.read_excel("Separacionsujetos.xlsx",skiprows=1,usecols="A,B,D")
    X = data_info["Sujeto"]
    Y = data_info["Izq"]
    X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size=119, random_state=None,stratify=Y)
    x_train, x_val, y_train, y_val = train_test_split(X_train, Y_train, test_size=75, random_state=42,stratify=Y_train)
...
os.mkdir("D:/EduardoCavieres/TesisMagister/Resultados/prep_datos_%s" %hora)
archivo = "D:/EduardoCavieres/TesisMagister/Resultados/prep_datos_%s/dataset_%s.txt" %(hora,hora)
...
f_salida = open("D:/EduardoCavieres/TesisMagister/Resultados/prep_datos_%s/out_%s.txt" %(hora,hora),"a+")
f_mask = open("D:/EduardoCavieres/TesisMagister/Resultados/prep_datos_%s/mask_size%s.txt" %(hora,hora),"a+")
...
##########################################################
    ...
    #########################
       ...
       ##############resize#################
        ...
        inputs_tmp_T1 = resize(inputs_tmp_T1, (128,128,128), order=1, mode='reflect', cval=0, clip=True, preserve_range=True, anti_aliasing=False, anti_aliasing_sigma=False)
        inputs_tmp_T2 = resize(inputs_tmp_T2, (128,128,128), order=1, mode='reflect', cval=0, clip=True, preserve_range=True, anti_aliasing=False, anti_aliasing_sigma=False)
        inputs_tmp_T1ce = resize(inputs_tmp_T1ce, (128,128,128), order=1, mode='reflect', cval=0, clip=True, preserve_range=True, anti_aliasing=False, anti_aliasing_sigma=False)
        inputs_tmp_Flair = resize(inputs_tmp_Flair, (128,128,128), order=1, mode='reflect', cval=0, clip=True, preserve_range=True, anti_aliasing=False, anti_aliasing_sigma=False)
        labels_tmp = resize( labels_tmp, (128,128,128), order=0, mode='reflect', cval=0, clip=True, preserve_range=True, anti_aliasing=False, anti_aliasing_sigma=False)
    #########################    
